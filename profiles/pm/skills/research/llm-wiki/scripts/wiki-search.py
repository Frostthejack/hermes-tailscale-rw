...
STATE_FILE = Path(os.environ.get("WIKI_SEARCH_STATE", str(Path.home() / ".wiki-search-state.json")))
...
def ingest_file(filepath: Path, state: dict, force: bool = False) -> bool:
    """Ingest a single wiki file into Hindsight."""
    ...
    tmp_path = f"/tmp/wiki_ingest_{filepath.stem}_{hash(str(rel_path)) % 10000}.md"
    Path(tmp_path).write_text(content)

    try:
        with open(tmp_path, "rb") as f:
            files = {"files": (str(rel_path), f, "text/markdown")}
            data = {
                "request": json.dumps({
                    "tags": ["wiki", str(rel_path.parent)] if str(rel_path.parent) != "." else ["wiki"],
                    "metadata": {
                        "source": "wiki",
                        "path": str(rel_path),
        ...
        if resp.status_code == 200:
            ...
            print(f"  ✓ Ingested: {rel_path}" + (f" (op: {op_id[:8]}...)" if op_id else ""))
