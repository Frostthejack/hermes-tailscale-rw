...
## Multi-Source Harvester Pattern

When you have 5+ sources to poll, use a YAML-driven orchestration script instead of individual cron jobs. See [references/yaml-harvester-pattern.md](references/yaml-harvester-pattern.md) for the full pattern including:

- `sources.yaml` structure (wiki_sources vs news_sources sections)
- Shell-safe delimiter choice (`§` — not tab or pipe)
- Harvester script architecture with `--dry-run` and `--full` flags
- State management (watermarks, pending files, daily caches)
- Google News RSS as proxy for towns without direct feeds

Working implementation: `~/.hermes/scripts/wiki-harvester.sh`

## Pitfalls

Small towns and cities rarely have working direct RSS feeds. Google News search RSS is a reliable proxy:

```bash
# Format for local news (city + state)
https://news.google.com/rss/search?q=City+State&hl=en-US&gl=US&ceid=US:en

# Format for state-level news
https://news.google.com/rss/search?q=State&hl=en-US&gl=US&ceid=US:en

# Format for specific outlet (when direct RSS is broken)
https://news.google.com/rss/search?q=site:outlet.com+City&hl=en-US&gl=US&ceid=US:en
```

These return Atom feeds, parsed by `watch_rss.py` (handles both RSS 2.0 and Atom).

## Verify Feeds Before Configuring

Many published RSS URLs are broken (404, 403, timeout). Always test before adding to production:
...
with urllib.request.urlopen(req, timeout=10) as r:
    ...
    print(f'OK: {root.tag}, {len(data)//1024}KB')
"
```

Common failures:
- **401/403**: Paywalled or bot-blocked. Use Google News proxy instead.
- **404**: Feed URL changed. Search for the new one or use Google News.
- **Timeout**: Outlet blocks slow connections. Usually means they don't want automated access.
- **Parse error**: Malformed XML. Try a different feed from the same outlet.
...
5. **Trusting published RSS URLs without testing.** Many are stale. Always verify with a direct HTTP request before wiring into cron.
