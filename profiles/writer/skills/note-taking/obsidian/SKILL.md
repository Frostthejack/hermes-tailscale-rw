...
```bash
# Primary vault (confirmed location)
OBSIDIAN_VAULT_PATH="/mnt/c/Users/luned/Vault/Encephalon-Mageia"

# WSL accessing Windows vault (general pattern)
OBSIDIAN_VAULT_PATH="/mnt/c/Users/<username>/Vault/<VaultName>"
```

**Important:** Always quote paths containing spaces.

**Confirmed vault location:** `/mnt/c/Users/luned/Vault/Encephalon-Mageia` — this is the primary and only vault. All project files, notes, and research live here.
...
```bash
# Switch vaults by changing the env var
export OBSIDIAN_VAULT_PATH="/mnt/c/Users/luned/Vault/Encephalon-Mageia"
```
...
**Always set user identity locally** before committing. WSL git won't have your GitHub identity by default.
...
The Obsidian Git plugin auto-commits from the local Windows machine, so the WSL git state often falls behind. When `git push` rejects:
...
10. **Vault is canonical**: All notes, projects, and documentation live in the vault at `/mnt/c/Users/luned/Vault/Encephalon-Mageia/`. When asked to create a file "in the vault" or "in a project folder," it MUST go under this path.
...
- **Files placed outside the vault**: When the user asks to create a file "in [project]" or "in the research folder," it MUST go in the Obsidian vault (`/mnt/c/Users/luned/Vault/Encephalon-Mageia/Projects/<Category>/<Project>/`), NOT in `C:\Users\luned\Documents\`. The vault is the single source of truth for all project files, notes, and research. Always verify the path starts with the vault root before writing.
