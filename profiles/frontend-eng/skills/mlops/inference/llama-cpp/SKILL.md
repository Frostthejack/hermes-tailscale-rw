...
When the user wants a local LLM server (not just CLI inference), guide them to Ollama first:
- **Ollama** = llama.cpp + model registry + auto GPU offload + service management. Use for always-on local servers.
- **Direct llama.cpp** = manual control. Use when the user needs specific flags, custom builds, or is debugging inference issues.
- **LM Studio** = llama.cpp + GUI. Use for browsing/testing models visually.

Don't recommend raw llama.cpp for "I want to run a local model" — recommend Ollama unless the user has a specific reason to go direct.
