...
① **Prefer semantic search via `ws`** — The `ws` CLI (wiki-search) is installed at `~/.local/bin/ws` and on PATH. Use it as the primary search tool:
   ```bash
   ws search "query" -n 5
   ws ingest              # Re-index after any wiki changes
   ws status              # Check bank health + ingestion state
   ```
   This uses Hindsight's built-in embeddings (mimir-well bank) for semantic recall — finds conceptually related content even without exact keyword matches. If `ws` is not found, fall back to `python3 ~/.hermes/scripts/wiki-search.py`.
...
If Hindsight API is running locally (default: `http://0.0.0.0:8888`), it can serve as a semantic search backend for the wiki — no separate embedding model needed. Hindsight's recall endpoint handles embedding and vector search internally.

**Setup:**
1. Create a dedicated bank: use an existing empty bank (e.g., `mimir-well`) or create a new one
2. Ingest wiki pages via the file retain endpoint
3. Search via the recall endpoint

**Reference implementation:** `ws` CLI (installed at `~/.local/bin/ws`, symlinked from `~/.hermes/scripts/wiki-search.py`) — a ready-to-use CLI that:
- `ws ingest` — reads all wiki `.md` files, submits to Hindsight, tracks file hashes for incremental updates
