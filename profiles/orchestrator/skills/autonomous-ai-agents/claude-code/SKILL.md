...
| `claude ultrareview [target]` | Run ultrareview non-interactively (e.g. `--from-pr 1234 --json`) |
| `claude daemon status` | Print supervisor state |
| `claude project purge [path]` | Delete local Claude Code state (`--dry-run` to preview) |
...
| `claude auto-mode defaults` | Print auto mode classifier rules (redirect to file for inspection) |
...
13. **OAuth token expires ~24h** — `claude auth status` shows `expiresAt` in milliseconds. If `claude -p` returns 401 despite `auth status` showing logged in, the token may have expired. Re-auth with `claude auth login` (requires browser interaction). This is separate from the OpenRouter API key used by Hermes profiles.
14. **`read_file` truncates long values** — API keys and tokens in config files may appear truncated (e.g. `sk-or-...2138`) when read via `read_file`, but the actual value on disk may be complete. Verify with `xxd` or Python byte-level reads if auth fails unexpectedly.
15. **Profile credential pool exhaustion** — If a worker profile's `~/.hermes/profiles/<profile>/auth.json` has `credential_pool` entries with `last_status: "exhausted"`, the dispatcher will skip spawning for that profile. The credential pool reads from `~/.hermes/.env` (sandbox-protected). Fix: user must manually edit the `.env` file and restart the gateway. See agent-lane skill's `references/credential-pool-exhaustion-pattern.md`.

## Kanban Lane Integration

For delegation through Hermes Kanban, use the **agent-lane** skill pattern. Key points:

1. **Always isolate in a worktree** — never run Claude directly in a shared checkout
2. **Hermes owns the Kanban lifecycle** — Claude never calls kanban_complete/kanban_block
3. **Reconcile diffs** — treat Claude output as untrusted patches until reviewed
4. **Run tests independently** — Claude-run tests are advisory
5. **Record metadata** — include `agent_lane` object in `kanban_complete`

For kanban lane delegation, prefer:
- **Print mode** (`claude -p`) for bounded, well-defined tasks
- **Interactive mode** (tmux) for multi-step iterative work
- **Subagents** (tmux lanes) for parallel workstreams

See the `agent-lane` skill for the full pattern including worktree setup, prompt
construction, monitoring, reconciliation checklist, and metadata schema.

## Pitfalls & Gotchas

### HTTP 401 with Truncated API Key (Profile-Specific)
- **Symptom**: Workers fail immediately with `HTTP 401: Missing Authentication header`. The request dump shows `Authorization: Bearer sk-or-v1...2138` — the key contains literal `...` dots and is only ~15 characters.
- **Root cause**: The credential resolution chain is masking/truncating the API key before sending the HTTP request.
- **Diagnosis**: Check `~/.hermes/profiles/<profile>/sessions/request_dump_*.json` — the `request.headers.Authorization` field will show the truncated key.
- **Fix**: Restart the gateway: `hermes gateway restart`. If that doesn't help, check the profile's `config.yaml` for a masked key and replace with the full key from `.env`.

### Print Mode Requires Explicit kanban_complete
- **Symptom**: Worker exits cleanly (rc=0) but never calls `kanban_complete`. Dispatcher logs `protocol_violation`.
- **Fix**: Include explicit instruction in the task prompt: "Hermes owns the Kanban lifecycle. You MUST call kanban_complete when done. Do not exit without calling it."
...
11. **Skill self-update** — After complex agent-CLI tasks, review this skill for missing subcommands, flags, or pitfalls. Patch immediately. A pass that finds nothing to update is a missed learning opportunity.
