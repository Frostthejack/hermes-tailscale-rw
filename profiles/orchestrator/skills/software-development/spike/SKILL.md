...
## Research-first for UI features

When the user wants to build a UI feature that has existing implementations in the wild (screen pets, chat interfaces, dashboards, etc.), **research existing implementations BEFORE writing any code.** This is not optional — it's the difference between building something that matches user expectations and building something that "just bounces and does nothing."

### Mandatory research steps:

1. **Find 2-3 existing implementations** of the feature. Search for:
   - Open source projects on GitHub (search "screen pet", "desktop pet", "egui pet", etc.)
   - Commercial products that do something similar
   - Framework-specific examples (egui demo apps, Tauri plugins, etc.)

2. **Analyze their interaction model.** For each implementation, document:
   - What can the user click/drag/hover?
   - What's click-through vs. interactive?
   - How do they handle window management (always-on-top, decorations, taskbar)?
   - What visual feedback do they provide?

3. **Identify the gap.** What does the user want that existing implementations don't have? What do existing implementations have that the user didn't ask for but would expect?

4. **Present findings to the user** before writing any code. Let them confirm the interaction model matches their vision.

### Anti-pattern — "just build it":
- Don't start coding based on your own assumptions about how a UI feature should work
- Don't build a "minimal version" and iterate — the user will be disappointed with the first iteration
- Don't skip research because "it's just a pet window" — interaction model matters more than visuals

### When this applies:
- Any desktop GUI feature (windows, overlays, system tray, notifications)
- Any feature with existing open-source implementations
- Any feature where the user references another product ("like aionui does it")
- Any feature where interaction model (click, drag, hover) is important
